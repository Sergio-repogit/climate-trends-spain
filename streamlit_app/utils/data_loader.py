...
def load_annual_timeseries(station_id):
    ...
    if df.empty:
        return df
    res = df[(df["dataset_type"] == "annual_timeseries") & (df["station_id"] == station_id)].dropna(axis=1, how="all")
    if not res.empty:
    ...
    for s in stations:
        ...
        for year in range(2010, 2026):
            ts_rows.append({
                "station_id": s,
                "year": year,
                "temp": base_temp + (year - 2010) * 0.035 + np.random.normal(0, 0.5),
    ...
    for s in stations:
        for year in range(2010, 2026):
            ext_rows.append({
                "station_id": s,
                "year": year,
                "is_tropical_night": int(np.random.poisson(10 + (year - 2010) * 0.5)),
